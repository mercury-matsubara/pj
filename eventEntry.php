<?php
	...
	$sql = "insert into calenderinfo (SUBJECT,MAINTEXT,STARTDATE,ENDDATE)values('".$title."','".$honbun."','".$start_year.$start_month.$start_day.$start_hour.$start_min."00','".$end_year.$end_month.$end_day.$end_hour.$end_min."00')";
	$con=dbconect();
//	$stmt=$con->prepare('SET NAMES utf8mb4');
//	$stmt->execute();
	$stmt=$con->prepare($sql) or error_log($con->error,0);
	$stmt->execute() or error_log($con->error,0);
	header("HTTP/1.1 301 Moved Permanently");
//	header("Location: http://localhost/leadplan/calendar.php");
